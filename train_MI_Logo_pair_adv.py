...
from MI_Net_logo import Mine, Recon_encoder_fusion, Recon_decoder_fusion, Seg_encoder_DNL,Seg_decoder_DNL
...
demo_path="./mutual_info_dataset"
test_demo_path="./mutual_info_dataset"
...
def update_Rec(inputs,GT,train):
    ...
    # rec_loss = F.mse_loss(Recon_result, inputs, reduction='mean')*(len(inputs))
...
def update_Seg_encoder(inputs_1, inputs_2, train):
    z_a_1, _, _, _, _ = Seg_encoder(inputs_1)
    z_a_2, _, _, _, _ = Seg_encoder(inputs_2)
    
    loss = F.mse_loss(z_a_1, z_a_2, reduction='sum')
...
def update_Ad_Seg(inputs, labels, train):
    
    z_a_1, _, _, _, _ = Seg_encoder(inputs)
    z_a_2, _, _, _, _ = Seg_encoder(1-labels)
...
def update_Ad_Rec(inputs, labels, train):
    # z_d_1 = Rec_encoder(inputs)
    z_d_2 = Rec_encoder(1-labels)

    loss = torch.sum(torch.std(z_d_2,dim=0))
...
def learn_mine(inputs, ma_rate=0.001):
    ...
    # mutual_information,_,_ = mi_estimator(z_a, z_d, z_d_shuffle)
